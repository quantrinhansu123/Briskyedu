...
**Auto-sync Behavior:**
- Sessions automatically generated by Cloud Function (`onClassCreate`, `onClassUpdate`) when class created/updated
- Regenerated when class schedule, totalSessions, or startDate changes
- **Data safety**: Sessions with `attendanceId` are preserved during regeneration (không xóa buổi đã điểm danh)
...
```typescript
{
  ...
  classId: string (reference to classes, indexed)
  className: string
  sessionNumber: number (sequential, 1-based)
  date: string (YYYY-MM-DD format, indexed)
  dayOfWeek: string ('T2' | 'T3' | 'T4' | 'T5' | 'T6' | 'T7' | 'CN')
  time: string (HH:mm-HH:mm format, e.g., "19:00-21:00")
  room: string | null
  teacherId: string | null (reference to staff)
  teacherName: string | null
  status: 'Chưa học' | 'Đang học' | 'Đã học' | 'Nghỉ lễ' | 'Hoãn'
  attendanceId?: string (reference to attendance, populated after điểm danh)
  createdAt: string (ISO timestamp, tracks when session was generated)
}
```

**Key Points:**
- `createdAt` field tracks when session was auto-generated by Cloud Function
- Sessions linked to attendance via `attendanceId` after teacher takes attendance
- Cloud Function preserves sessions with `attendanceId` during schedule changes to prevent data loss
