/**
 * Check Starters 25 class sessions
 * Verify: order, dates match schedule, no gaps/duplicates
 *
 * Usage: npx tsx scripts/check-starters25-sessions.ts
 */
...
async function checkStarters25() {
  // 1. Find the class
  const classSnap = await db.collection('classes').where('name', '==', 'Starters 25').get();

  if (classSnap.empty) {
    // Try partial match
    const allClasses = await db.collection('classes').get();
    const matches = allClasses.docs.filter(d =>
      (d.data().name || '').toLowerCase().includes('starters 25')
    );
    if (matches.length === 0) {
      console.log('❌ Không tìm thấy lớp "Starters 25"');
      console.log('Các lớp có "starters":', allClasses.docs
        .filter(d => (d.data().name || '').toLowerCase().includes('starter'))
