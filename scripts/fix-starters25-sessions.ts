/**
 * Fix Starters 25 sessions
 * - Renumber all sessions sequentially by date (1, 2, 3, ...)
 * - Standardize dayOfWeek format ("Thứ 2" → "Thứ Hai")
 *
 * Usage:
 *   npx tsx scripts/fix-starters25-sessions.ts          # Dry run
 *   npx tsx scripts/fix-starters25-sessions.ts --execute # Apply changes
 */
...
const CLASS_ID = 'xVM8f3paoEcaUuFB0GTk'; // Starters 25
...
async function fixStarters25() {
  ...
  if (!IS_DRY_RUN) {
    ...
    console.log('   npx tsx scripts/fix-starters25-sessions.ts --execute');
  }
}
