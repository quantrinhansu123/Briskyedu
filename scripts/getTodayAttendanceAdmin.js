/**
 * Get Today's Attendance Data (using Admin SDK)
 * Lấy dữ liệu điểm danh của ngày hôm nay
 * Run: node scripts/getTodayAttendanceAdmin.js
 */
...
async function getTodayAttendance() {
  ...
  try {
    ...
    groupedByAttendance.forEach((students, attendanceId) => {
      const attendanceRecord = attendanceRecords.find((r) => r.id === attendanceId);
      console.log(`\n${'─'.repeat(80)}`);
      ...
      byStatus.forEach((studentsWithStatus, status) => {
        ...
        studentsWithStatus.forEach((student, idx) => {
    ...
    const withSessionId = studentAttendanceRecords.filter((r) => r.sessionId).length;
    const withoutSessionId = studentAttendanceRecords.filter((r) => !r.sessionId).length;
